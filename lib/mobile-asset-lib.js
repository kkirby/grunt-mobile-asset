...
var ProcessIcon = function () {
	var _ref4 = (0, _asyncToGenerator3.default)(_regenerator2.default.mark(function _callee2(iconName, config, outputFolder, gmConfig) {
		var scale, type, name, width, height, resize, resizeFlags, resizeWidth, resizeHeight, backgroundColor, gravity, inputImageDimensions, tempIconName, imageData, tempTransformHolder, _iteratorNormalCompletion, _didIteratorError, _iteratorError, _iterator, _step, transformFile, iconInstance, resizeFlag, aspect;
		...
		return _regenerator2.default.wrap(function _callee2$(_context2) {
			while (1) {
				switch (_context2.prev = _context2.next) {
					case 0:
						uniqid++;
						scale = config.scale, type = config.type, name = config.name, width = config.width, height = config.height, resize = config.resize, resizeFlags = config.resizeFlags, resizeWidth = config.resizeWidth, resizeHeight = config.resizeHeight, backgroundColor = config.backgroundColor, gravity = config.gravity;
						inputImageDimensions = imageCache(iconName, function () {
							return (0, _imageSize2.default)(iconName);
						});
					...
					case 37:
						...
						aspect = inputImageDimensions.width / inputImageDimensions.height;
						...
						iconInstance.background(backgroundColor).resize(resizeWidth * scale, resizeHeight * scale, resizeFlag).gravity(gravity).extent(width * scale, height * scale);
						_context2.next = 47;
